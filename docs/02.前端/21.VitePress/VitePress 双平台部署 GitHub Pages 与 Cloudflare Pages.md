...
- **默认值** `/notes/` —— GitHub Pages 模式（现有行为不变）
- **设置 `VITEPRESS_BASE=/`** —— Cloudflare Pages 模式

这样同一个配置文件能产出两套不同 base 的构建产物，平台各自拿自己的版本。

### 代码改动

修改 `docs/.vitepress/config.ts`，在文件头部新增 base 和 siteUrl 变量：
...
::: tip 向后兼容
不设置 `VITEPRESS_BASE` 时默认走 `/notes/`，现有 GitHub Pages 部署完全不受影响。
:::
...
| 检查项 | GitHub Pages 模式 | Cloudflare Pages 模式 |
|--------|------------------|----------------------|
...
```
git push → main
├─ GitHub Actions → 构建（base=/notes/）→ GitHub Pages
└─ Cloudflare Pages → 构建（base=/）     → notes.laplacesc.com
```
